...
const ICON_BG = "var(--color-accent)";
const ICON_GLYPH = "var(--white-fixed)";

/** Mor squircle + beyaz glyph — Figma 3765:11908. */
...
const PLATFORMS: { key: PlatformKey; label: string; Icon: () => ReactElement }[] = [
...
function socialAria(messages: Messages, platform: string, hasUrl: boolean) {
...
export function ContactSocials({
  ...
  return (
    <ul className="contact-socials" aria-label={messages.contact.socialsAria}>
      {PLATFORMS.map(({ key, label, Icon }) => {
        const href = settings[key].trim();
        const hasUrl = Boolean(href);
        const ariaLabel = socialAria(messages, label, hasUrl);
        const className = `contact-social-btn${
          hasUrl ? "" : " contact-social-btn--muted"
        }`;
        return (
          <li key={label}>
            {hasUrl ? (
